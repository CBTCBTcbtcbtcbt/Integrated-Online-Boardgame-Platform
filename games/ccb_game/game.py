...
class CCBGame(BaseGame):
    def __init__(self, room_id, rows=23, cols=23, command_point=0):
        ...
        self.spawn = [[12,11],[10,5],[10,21],[15,7]]
        for i in range(len(self.spawn)):
            self.board[self.spawn[i][0]][self.spawn[i][1]] = [i+1,0]
    ...
    def update_board_information(self):
        ...
        for sid in self.players:
            ...
            self.players[sid][3]=4+int(well_number*self.well_yield)
            print(well_number*self.well_yield)
        self.check_win()
        for (i,j) in self.citys:
